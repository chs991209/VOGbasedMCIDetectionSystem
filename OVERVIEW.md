...
4. [Feature Representation: 4-Channel Binocular Scalogram](#4-feature-representation-4-channel-binocular-scalogram)
...
### 3.6 Scale Grid and Artifact Rejection

Scales are derived from the desired frequency grid $\{f_j\}_{j=1}^{F}$ (**logarithmically** spaced from 1 to 40 Hz, $F = 40$ bins):

$$f_j = 10^{\log_{10}(f_{\min}) + \frac{j-1}{F-1}\bigl(\log_{10}(f_{\max}) - \log_{10}(f_{\min})\bigr)}, \qquad a_j = \frac{f_c}{f_j \cdot \Delta t}$$

Logarithmic spacing allocates more frequency bins at low frequencies, consistent with the $1/f$ power law of neural oscillations and providing finer resolution in the clinically informative 1–10 Hz latency band.
...
**Artifact rejection** is applied after epoching: any epoch where the peak absolute gaze error exceeds 30° in either eye is discarded before CWT computation. This removes blink artefacts and tracking failures that would otherwise introduce outlier power values across the full scalogram.

---

## 4. Feature Representation: 4-Channel Binocular Scalogram

Rather than collapsing the complex CWT to a real-valued power spectrum $|W_\psi|^2$, the pipeline retains the full complex output and processes **both eyes independently**, yielding a 4-channel tensor per epoch:

$$\mathbf{Z}_k = \begin{bmatrix} \operatorname{Re}(W_\psi[\tilde{e}_k^L]) \\ \operatorname{Im}(W_\psi[\tilde{e}_k^L]) \\ \operatorname{Re}(W_\psi[\tilde{e}_k^R]) \\ \operatorname{Im}(W_\psi[\tilde{e}_k^R]) \end{bmatrix} \in \mathbb{R}^{4 \times F \times T}$$

where superscripts $L$, $R$ denote the left and right eye gaze error signals respectively.

**Motivation for retaining phase.** The real and imaginary parts encode complementary projections onto cosine and sine basis functions. Their ratio determines the **instantaneous phase** $\phi(f, t) = \arctan(\operatorname{Im}/\operatorname{Re})$, which distinguishes:
...
**Motivation for binocular processing.** MCI-related oculomotor deficits can manifest asymmetrically between eyes (conjugate gaze disorders, monocular tracking degradation). Providing both eyes as separate channel pairs allows the network to detect inter-ocular asymmetry as an additional discriminative feature.

### Normalization

Each of the 4 channels is normalized independently per epoch via Z-score:

$$\hat{Z}_k^{(c)} = \frac{Z_k^{(c)} - \mu_k^{(c)}}{\sigma_k^{(c)} + \varepsilon}, \quad c \in \{1, 2, 3, 4\}, \quad \varepsilon = 10^{-8}$$

Per-channel normalization prevents the typically larger-magnitude real channels from dominating the imaginary channels during gradient updates, and prevents the dominant eye from suppressing the weaker eye's signal.
...
The input tensor $\mathbf{Z}_k \in \mathbb{R}^{4 \times 40 \times 100}$ is treated as a 4-channel image in the frequency-time plane. A convolutional architecture is appropriate because:
...
Blocks 2 and 3 include a **residual skip connection** via a $1 \times 1$ convolution that matches the output channel dimension:

$$\mathbf{x}^{(\ell+1)} = \text{ReLU}\!\left(f_\text{main}^{(\ell)}(\mathbf{x}^{(\ell)}) + f_\text{skip}^{(\ell)}(\mathbf{x}^{(\ell)})\right)$$

where $f_\text{skip}^{(\ell)}$ is a $1 \times 1$ conv + BN (+ MaxPool where spatial downsampling occurs). Residual connections improve gradient flow in the presence of early stopping and small batch sizes — both characteristic of the training regime here.
...
|-------|-------------|:---:|
| Input | `[B, 4, 40, 100]` | — |
| Block 1: Conv(4→16) + BN + ReLU + MaxPool | `[B, 16, 20, 50]` | 592 |
| CBAM 1: Channel(16) + Spatial(7×7) | `[B, 16, 20, 50]` | 386 |
| Block 2: DW(16)+PW(16→32)+BN+MaxPool + skip | `[B, 32, 10, 25]` | 1,136 |
| CBAM 2: Channel(32) + Spatial(7×7) | `[B, 32, 10, 25]` | 1,346 |
| Block 3: DW(32)+PW(32→64)+BN + skip | `[B, 64, 10, 25]` | 4,288 |
| CBAM 3: Channel(64) + Spatial(7×7) | `[B, 64, 10, 25]` | 4,994 |
| GAP → Flatten | `[B, 64]` | — |
| Dropout(0.3) + FC(64→32) + ReLU | `[B, 32]` | 2,080 |
| FC(32→2) | `[B, 2]` | 66 |
| **Total** | | **~11,832** |
...
### 7.1 Data Augmentation: SpecAugment

To improve generalization on the small dataset, **SpecAugment-style masking** is applied on-the-fly to training samples only (never to validation or inference inputs):

- **Frequency masking:** a contiguous band of 20% of frequency bins is zeroed at a random position
- **Time masking:** a contiguous window of 20% of time bins is zeroed at a random position

These augmentations are implemented in `AugmentedSubset`, which wraps a `torch.utils.data.Subset`. Each training epoch sees a different random masking pattern, effectively regularising the model against over-reliance on any single time-frequency region — analogous to dropout in the feature domain.

### 7.2 Subject-Level Stratified Split
...
### 7.3 Class-Weighted Cross-Entropy Loss
...
The cross-entropy loss uses `label_smoothing=0.1`, which replaces the hard one-hot targets with soft targets:

$$y_{\text{smooth}} = (1 - \epsilon)\,y_{\text{hard}} + \frac{\epsilon}{K}, \quad \epsilon = 0.1,\; K = 2$$

This prevents the model from driving softmax outputs to extreme values, improving calibration of the output probabilities — critical for the soft-voting ensemble and threshold optimisation in LOSO-CV.
...
### 7.6 Learning Rate Schedule: Cosine Annealing
...
Training uses early stopping with `patience=10`: if the validation loss does not improve for 10 consecutive epochs, training halts and the checkpoint with the lowest validation loss is restored. This prevents overfitting on small validation sets and avoids unnecessary computation in LOSO-CV folds.

### 7.8 Mixed-Precision Training (FP16)
...
where $\mathbf{z}_k$ is the normalized 4-channel binocular CWT tensor for epoch $k$.
...
### 9.1 Dataset Summary (Trial 1 — Local Cohort, Mac M2 Pro)

| Group | Subjects | Epochs (after artifact rejection) |
|-------|:---:|:---:|
| HC | 14 | 2,495 |
| MCI (incl. MCI+) | 23 | 3,241 |
| **Total** | **37** | **5,736** |

278 of 300 CSV files yielded valid binocular epochs. 22 files were skipped due to missing column headers or no extractable events.

### 9.2 Single-Split Training (80/20 subject-level split)

- Device: Apple MPS (FP32)
- Early stopped at epoch 22 (patience=10)
- Best validation loss: 0.6517 (~64% val accuracy)
- Checkpoint saved: `src/best_edge_cwt_model.pth`

### 9.3 LOSO-CV Results (37-fold, N=37 subjects)

| Metric | Threshold = 0.50 | Threshold = 0.52 (optimised) |
|--------|:---:|:---:|
| Accuracy | 67.6% (25/37) | 67.6% (25/37) |
| Sensitivity | **78.3%** (TP=18, FN=5) | 69.6% (TP=16, FN=7) |
| Specificity | 50.0% (TN=7, FP=7) | **64.3%** (TN=9, FP=5) |
| Balanced Accuracy | 64.1% | **66.9%** |
| AUROC | **0.683** | — |

Threshold optimisation scans $t \in [0.10, 0.90]$ in steps of 0.0125 to maximise balanced accuracy.

**Interpretation:** MCI soft-vote probabilities cluster near 0.5 (range 0.507–0.732), indicating the model is under-confident. This is a direct consequence of N=37 being insufficient to train a well-calibrated binary classifier from scratch. Results are expected to improve substantially with the additional ~37-subject fine-tuning cohort.

### 9.4 Output Files

| File | Description |
|------|-------------|
| `xai_difference_map.png` | SPM-style MCI−HC contrast (log-freq, binocular power) |
| `loso_results.png` | ROC curve + per-subject MCI probability strip |
| `src/best_edge_cwt_model.pth` | 4-channel model checkpoint (~11,832 params) |
| `pipeline_run.log` | Full stdout log of Trial 1 |
...
With $N = 37$ local subjects, LOSO-CV produces unbiased but high-variance estimates. Each fold's decision boundary is estimated from 36 subjects — marginal for a 11K-parameter model. The planned addition of ~37 fine-tuning subjects (→ N≈74 total) should meaningfully reduce variance and improve AUROC.

### 10.2 Binocular Signal Quality

The 4-channel binocular model adds expressiveness but also noise: if one eye's tracking is systematically degraded (e.g., asymmetric pupil size affecting the VOG sensor), the additional channels may hurt more than help at N=37. A per-subject eye-quality screening step is a natural extension.

### 10.3 Epoch Independence Assumption

The soft-voting ensemble assumes conditional independence of epochs from the same subject. In reality, successive saccade epochs within a trial are likely positively correlated (shared fatigue, attention, and baseline drift). Accounting for within-subject correlation — e.g., via a mixed-effects model or hierarchical Bayesian approach — is a natural extension for larger cohorts.

### 10.4 Formal XAI Thresholding
...
2. **Cluster-based FWER correction:** identify connected clusters where $|\Delta(f, t)| > \delta_0$ and threshold by cluster size under the null — the standard approach in SPM12/FieldTrip for electrophysiological data

### 10.5 Toward a Foundation Model

With the additional fine-tuning cohort, a two-stage approach is recommended:
1. **Pre-training** on all available data to learn a generalizable CWT-feature extractor
2. **Fine-tuning** on site-specific cohorts by freezing the convolutional backbone and retraining the classifier head (FC layers only)

This mirrors standard transfer learning practice and is expected to substantially improve generalization given small per-cohort sample sizes.

### 10.6 Jetson AGX Orin Deployment

The next planned step is deployment on a Jetson AGX Orin (Ampere GPU, CUDA):
- Re-run LOSO-CV with CUDA/FP16 AMP for direct comparison against the MPS/FP32 Mac baseline
- Export `best_edge_cwt_model.pth` to TensorRT via ONNX (`trtexec`) for real-time inference
- Benchmark epoch-level inference latency: FP16 TensorRT vs. plain PyTorch FP32
